...
# Converting public keys to PEM format for easy transmission and storage
client_public_keys_pem = {
    client_id: key.to_pem().decode() for client_id, key in client_public_keys.items()
}

#html for client's browser. This contains a basic chat interface and handles web socket communication
html_template = """
<!DOCTYPE html>
<html>
    <head>
...
@app.get("/{client_id}") #this is a route decorator that tells FastAPI to execute the GET request is made with the client id.
...
async def websocket_endpoint(websocket: WebSocket, client_id: int):
    ...
    try:
        while True:
            ...
            # Data is expected to be a JSON string that includes the signed message and the signature
            data_json = json.loads(data)
            message = data_json["message"]
            signature = base64.b64decode(data_json["signature"])

            # Verify the signature using the client's stored public key
            try:
                client_public_key = client_public_keys[client_id]
                if client_public_key.verify(signature, message.encode()):
                    # Signature is valid, broadcast the message
                    await manager.broadcast(f"Client #{client_id} says: {message}")
                else:
                    # Signature is invalid, send an error message back to the client
                    await manager.send_personal_message("Invalid signature.", client_id)
            except Exception as e:
                # Handle any exception that occurs during verification
                await manager.send_personal_message(f"Signature verification failed: {str(e)}", client_id)
